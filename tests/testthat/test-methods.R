...
test_that("subsetting and concatenating works", {
  ...
  M1 <- marginal(fit, v = xvars[1:2], data = iris, y = "Sepal.Length", breaks = 5)
  M2 <- marginal(fit, v = xvars[3:4], data = iris, y = "Sepal.Length", breaks = 5)
...
test_that("object is of class 'marginal'", {
  expect_s3_class(M, "marginal")
})
